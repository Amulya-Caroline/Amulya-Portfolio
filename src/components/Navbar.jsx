...
const Navbar = () => {
  ...
  return (
    ...
    <nav ref={navRef} className="w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50">
        ...
        className="w-32 cursor-pointer mr-14 dark:hidden" />

        <img src={logo_darkk} alt="Logo"
        className="hidden w-32 cursor-pointer mr-14 dark:block" />

        <ul ref={navLinkRef} className="items-center hidden gap-6 px-12 py-3 bg-white bg-opacity-50 rounded-full shadow-sm md:flex lg:gap-8 font-Ovo dark:border dark:border-white/50 dark:bg-transparent ">
        ...
        <a href="#contact" 
        className="hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 font-Ovo dark:border-white/50 hover:bg-gradient-to-r hover:from-[#b820e6] hover:to-[#da7d20] hover:text-white transition-all duration-300 group">

        Contact 
        <img src={arrow_icon} alt="" className="w-3 dark:hidden group-hover:hidden" />
        <img src={arrow_icon_dark} alt="" className="hidden w-3 dark:block group-hover:block" />
...
{/* -- ----- mobile menu ------  -- */}
        <ul ref={sideMenuRef} className="fixed top-0 bottom-0 z-50 flex flex-col w-64 h-screen gap-4 px-10 py-20 transition duration-500 md:hidden -right-64 bg-rose-50 font-Ovo dark:bg-darkHover dark:text-white">
            ...
            <div className="absolute right-6 top-6" onClick={closeMenu}>
                ...
                <img src={close_white} alt="" className="hidden w-5 cursor-pointer dark:block" />
            </div>
